...
function Rooibos__Init(preTestSetup = invalid, testUtilsDecoratorMethodName = invalid, testSceneName = invalid, nodeContext = invalid) as void
  ...
  ? "Starting test using test scene with name " ; testSceneName
...
sub RBS_LOGGER_PrintTestStatistic(testCase as object)
  ...
  if (metaTestcase.isParamTest <> true)
    messageLine = RBS_LOGGER_FillText(" " + testChar + " |--" + metaTestCase.Name + " : ", ".", 60)
  ...
  if (metaTestcase.isParamTest = true)
    ...
    messageLine = m.fillText(" " + testChar + insetText + " |--" + formatJson(rawParams) + " : ", ".", 60)
    ? messageLine ; testCase.Result ; timeText
  end if
  if LCase(testCase.Result) <> "success"
...
sub Rooibos_run()
  ...
  for each metaTestSuite in m.runtimeConfig.suites
    ...
    ? ""
    ? RBS_LOGGER_FillText("> SUITE: " + metaTestSuite.name, ">", 60)
...
sub Rooibos_RunItGroups(metaTestSuite, totalStatObj, testUtilsDecoratorMethodName, config, runtimeConfig, nodeContext = invalid)
  ...
  for each itGroup in metaTestSuite.itGroups
    ...
    ? ""
    ? RBS_LOGGER_FillText("> GROUP: " + itGroup.name, ">", 60)
...
sub Rooibos_RunTestCases(metaTestSuite, itGroup, testSuite, totalStatObj, config, runtimeConfig)
  ...
  for each testCase in testSuite.testCases
    ...
    ? ""
    ? RBS_LOGGER_FillText("> TEST: " + testCase.Name + " ", ">", 60)
...
sub Rooibos_SendHomeKeypress()
  ...
  ut.SetUrl("http://localhost:6060/keypress/Home")
  ut.PostFromString("")
end sub
